...
func _display_results() -> void:
	...
	for i in sorted_players.size():
		...
		if p["name"] == GameData.players[GameData.winner_index]["name"]:
		...
		match GameData.game_mode:
			...
			GameData.GameMode.CRICKET:
				...
				for n in GameData.CRICKET_NUMBERS:
					marks_str += "%d:%s  " % [n, _marks_icon(p["cricket_marks"][n])]
				marks_lbl.text = marks_str.strip_edges()
				marks_lbl.add_theme_font_size_override("font_size", 11)
				marks_lbl.add_theme_color_override("font_color", Color(0.6, 0.6, 0.6))
			GameData.GameMode.FREE_SCORE:
		...
		if GameData.game_mode == GameData.GameMode.CRICKET:
			...
			marks_lbl.add_theme_font_size_override("font_size", 11)
			marks_lbl.add_theme_color_override("font_color", Color(0.55, 0.55, 0.55))
			marks_row.add_child(marks_lbl)
			scores_container.add_child(marks_row)
		...
		if history.size() > 0:
			...
			scores_container.add_child(stat_lbl)
